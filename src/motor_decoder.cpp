...
MotorDecoder::MotorDecoder(uint pin_a, uint pin_b) : _pin_a{pin_a}, _pin_b{pin_b} {
  ...
  interrupts::register_callback(
    _pin_a, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, [this] { this->on_a_change(); }
  );
  interrupts::register_callback(
    _pin_b, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, [this] { this->on_b_change(); }
  );
}

MotorDecoder::~MotorDecoder() {
  interrupts::unregister_callback(_pin_b, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE);
