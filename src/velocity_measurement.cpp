...
VelocityMeter::VelocityMeter(
  ...
  : _decoder{std::move(decoder)}, _speed_factor{1.0 / (ticks_per_revolution * gear_ratio)} {}
...
float VelocityMeter::compute_speed_tps() {
  ...
  if (delta_time == 0 || delta_time > max_measurement_interval) return 0.0;
...
float VelocityMeter::compute_speed_rpm() { return compute_speed_rps() * 60.0; }
